...
5. **Docker images** (automatic if CI is configured)  
   Pushing a version tag (e.g. `git push origin v1.0.1`) triggers the [Docker release workflow](.github/workflows/docker-release.yml): the image is built and pushed to **Docker Hub** (`DOCKERHUB_USERNAME/php-qrcodegenerator`) and **GHCR** (`ghcr.io/OWNER/REPO`).  
   Add repo secrets: `DOCKERHUB_USERNAME`, `DOCKERHUB_TOKEN` (Docker Hub access token). GHCR uses `GITHUB_TOKEN` automatically.  
   To build and push manually: set `DOCKERHUB_IMAGE` and `GHCR_IMAGE`, then run `./scripts/docker-release.sh [VERSION]` (script is in `scripts/`, make executable with `chmod +x` if needed).

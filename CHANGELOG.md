...
- **Admin panel reorganized** — The admin page is now split into three sections with pill navigation: **Updates** (GitHub repo for zip installs), **Authentication** (IP allowlist, allow app from any IP, login options, upgrade secret, admin secret), and **Modules** (custom modules and default preset visibility/order).
